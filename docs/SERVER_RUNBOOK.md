...
Создайте два unit-файла и включите сервисы — не нужно держать открытыми окна SSH.
...
Для продакшена лучше завести отдельного пользователя Linux (не `root`) и выдать права на `/srv/tg_mini_app` — сейчас шаблон упрощённый.
