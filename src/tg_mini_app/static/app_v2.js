...
async function main() {
  ...
  const checkoutHint = byId("checkoutHint");
  const checkoutBtn = byId("checkoutBtn");
  if (!checkoutHint || !checkoutBtn) return;
  ...
  checkoutBtn.addEventListener("click", async () => {
    ...
    checkoutHint.textContent = "Отправляю заказ…";
    ...
    try {
      ...
      if (!cart.items.length) {
        checkoutHint.textContent = "Добавьте позиции в корзину.";
        return;
      }
      if (!address || !deliveryTime) {
        checkoutHint.textContent = "Заполните адрес и время доставки.";
        return;
      }
      ...
      if (!customerTgId) {
        ...
        checkoutHint.textContent =
          "Не удалось определить Telegram ID. Введите его вручную (для отладки) и нажмите ещё раз.";
        return;
      }

      checkoutHint.textContent = `Отправляю заказ… (tg_id=${customerTgId})`;
      ...
      checkoutHint.textContent = `Заказ #${order.id} отправлен на согласование. Ожидайте ответ в чате Telegram.`;
    } catch (e) {
      checkoutHint.textContent = `Ошибка при отправке: ${String(e)}`;
    } finally {
